...
import static mysh.cluster.FilesMgr.FileType.CORE;
...
class Worker implements IWorker {
            ...
            try {
                while (keepRunning && !isInterrupted()) {
                    try {
                        ...
                        log.error("run-sub-task-error. " + currSubTask, e);
                    }
                }
            } finally {
        ...
        public void run() {
            ...
            while (!this.isInterrupted()) {
                try {
                    ...
                    Throwable nodeUnavailableExp = ClusterNode.getNodeUnavailableExp(e);
                    if (nodeUnavailableExp != null) {
                        if (task.masterId != null)
                            masterServiceUnavailable(task.masterId);
                        log.error("submit-task-result-error:master-is-unavailable: task={}, exp={}",
                                task, String.valueOf(nodeUnavailableExp));
                    } else
    ...
    public void cancelTask(int taskId, int subTaskId) {
        log.info("cancel-task-request,taskId=" + taskId + ",subTaskId=" + subTaskId);
        this.lastMasterAction = System.currentTimeMillis();
    ...
    private class SubTask<T, ST, SR, R> {
        ...
        public void execute() {
            try {
                if (System.currentTimeMillis() > this.execBefore) {
                    ...
                    log.info("run-subTask:" + this.toString());
                    result = cUser.doPrivilegeOp(
                            () -> cUser.procSubTask(subTask, timeout));
                    log.info("run-subTask-successfully:" + this.toString());
                }
            } catch (Throwable e) {
                result = e;
        ...
        public void cancel() {
            log.info("subTask-canceled: " + this.toString());
            this.execBefore = 0;
        }

        private transient String thisStr;
    ...
    public void updateFiles(String dispatcherId, String thumbStamp) {
        if (clusterNode.getNodeState() != ClusterNode.ClusterState.WORKER)
            return;

        final FilesInfo currFilesInfo = filesMgr.getFilesInfo();
        if (!currFilesInfo.thumbStamp.equals(thumbStamp)) {
            log.info("begin-to-update-files-from:{}, ts:{}", dispatcherId, thumbStamp);
